...
def test_render_emits_one_text_element_per_row():
    svg = render({"cols": 4, "rows": 3, "ink": [[0.0] * 4 for _ in range(3)]})
    assert svg.count('class="row') == 3


def test_every_row_forces_its_width():
    svg = render({"cols": 4, "rows": 3, "ink": [[0.5] * 4 for _ in range(3)]})
...
def test_render_has_no_script():
    svg = render({"cols": 4, "rows": 2, "ink": [[0.3] * 4 for _ in range(2)]})
